...
import { Volume2, Mic, Clock, LogOut, Settings, Play, Loader2, Languages, Phone, CreditCard, Sparkles, TrendingUp, History } from 'lucide-react'
      ...
      <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {userData?.subscription ? (
          <div className="mb-8 flex flex-wrap gap-4 animate-fade-in animate-delay-100">
            <Button onClick={handleGenerate} size="lg" className="gap-2 gradient-primary border-0 transition-bounce hover:scale-105 hover:shadow-lg hover:shadow-primary/25">
              <Mic className="h-5 w-5" />
              Generate New Voice
            </Button>
            <Button onClick={handleVoiceClones} size="lg" variant="outline" className="gap-2 hover:border-primary/50 hover:bg-primary/5 transition-all">
              <Languages className="h-5 w-5" />
              Manage Voice Clones
            </Button>
            <Button onClick={() => router.push('/history')} size="lg" variant="outline" className="gap-2 hover:border-primary/50 hover:bg-primary/5 transition-all">
              <History className="h-5 w-5" />
              View History
            </Button>
          </div>
        ) : (
          <Card className="mb-8 glass-card border-amber-500/30 animate-fade-in animate-delay-100">
            <CardContent className="pt-6">
              <div className="flex items-start gap-4">
                <div className="h-12 w-12 rounded-2xl bg-gradient-to-br from-amber-500 to-orange-500 flex items-center justify-center flex-shrink-0 shadow-lg">
                  <CreditCard className="h-6 w-6 text-white" />
                </div>
                <div>
                  <h3 className="font-semibold text-amber-900 dark:text-amber-100">Subscription Required</h3>
              ...
              {userData?.subscription ? (
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-4 rounded-xl bg-gradient-to-r from-primary/10 to-purple-500/10 border border-primary/20">
                    ...
                    <Badge variant={userData.subscription.status === 'active' ? 'default' : 'secondary'} className={userData.subscription.status === 'active' ? 'gradient-primary border-0' : ''}>
                    ...
                    <ul className="space-y-2">
                      {Object.entries(userData.subscription.plan.features).map(([feature, included]) => (
                        <li key={feature} className="flex items-center gap-3 text-sm">
                          <div className={`h-2 w-2 rounded-full ${included ? 'bg-gradient-to-r from-emerald-500 to-green-500' : 'bg-muted-foreground/30'}`} />
                  ...
                  {userData.user.role === 'admin' && (
                    <Button variant="outline" className="w-full mt-4 hover:border-primary/50 hover:bg-primary/5" onClick={() => router.push('/admin')}>
                      Go to Admin Panel
                    </Button>
                  )}
                </div>
            ...
            <CardContent>
              {generations.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <Button className="gradient-primary border-0 hover:shadow-lg hover:shadow-primary/25 transition-all" onClick={handleGenerate}>
                ...
                <div className="space-y-4">
                  {generations.map((generation, index) => (
                    <div 
                      key={generation.id} 
                      className="p-3 sm:p-4 rounded-xl border border-border/50 bg-muted/30 space-y-3 hover:border-primary/30 hover:bg-muted/50 transition-all duration-300 cursor-pointer"
                      style={{ animationDelay: `${index * 100}ms` }}
                    >
                      <div className="flex items-start gap-2 sm:gap-4">
                        <div className="h-8 w-8 sm:h-10 sm:w-10 rounded-lg sm:rounded-xl bg-gradient-to-br from-primary/20 to-purple-500/20 flex items-center justify-center flex-shrink-0">
                  ...
                  <Button variant="outline" className="w-full hover:border-primary/50 hover:bg-primary/5 transition-all text-sm" onClick={() => router.push('/history')}>
                    View All History
                  </Button>
                </div>
              )}
